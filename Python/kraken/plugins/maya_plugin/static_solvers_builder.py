...
class Builder(Builder):
    ...
    @property
    def currentRigGraph(self):
        return self.__currentRigGraph
    ...
    def buildSkeleton(self, kSceneItem, buildName):
        ...
        self._registerSceneItemPair(kSceneItem, dccSceneItem)
        self.addAbstractBone(dccSceneItem)
    ...
    def buildOrientationConstraint(self, kConstraint, buildName):
        ...
        if self.isConstrainPartOfComponentIO(kConstraint):
            cns = self.buildNodeConstraint(kConstraint, buildName)
            self.__abstractConstraints.append(cns)
            self._registerSceneItemPair(kConstraint, cns.rigGraph.getCanvasNode())

        else:
            cns = self.buildDccOrientationConstraint(kConstraint, buildName)
    ...
    def buildPoseConstraint(self, kConstraint, buildName):
        ...
        if self.isConstrainPartOfComponentIO(kConstraint):
            ...
            cns = self.buildDccPoseConstraint(kConstraint, buildName)
    ...
    def buildPositionConstraint(self, kConstraint, buildName):
        ...
        if self.isConstrainPartOfComponentIO(kConstraint):
            ...
            cns = self.buildDccPositionConstraint(kConstraint, buildName)
    ...
    def buildScaleConstraint(self, kConstraint, buildName):
        if self.isConstrainPartOfComponentIO(kConstraint):
            ...
            cns = self.buildDccScaleConstraint(kConstraint, buildName)

        return cns

    def buildNodeConstraint(self, kConstraint, buildName):
        self.buildCanvasContainer("{}Canvas".format(buildName))
        cns = CanvasConstraint(self, kConstraint, buildName, self.currentRigGraph)
        return cns
    ...
    def buildKLOperator(self, kOperator, buildName):
        ...
        # to true.
        op = self.buildCanvasOperator(kOperator, buildName, isKLBased=True)
    ...
    def buildCanvasOperator(self, kOperator, buildName, isKLBased=False):
        ...
        """
        self.buildCanvasContainer("{}Canvas".format(buildName))
        op = CanvasOperator(self, kOperator, buildName, self.currentRigGraph, isKLBased)
        self.__klCanvasOps.append(op)
        return op

    def buildSkeletonContainer(self, buildName):

        self.buildCanvasContainer("{}Canvas".format(buildName))
        self._characterSkeleton = AbstractSkeleton(buildName, self.currentRigGraph)
    ...
    def buildCanvasContainer(self, buildName):
        ...
        self.__currentRigGraph = MayaGraphManager(buildName)
        self.currentRigGraph.setTitle('Rig')
        self.currentRigGraph.addExtDep('Kraken')
        self.currentRigGraph.getOrCreateArgument("drawDebug", dataType="Boolean", defaultValue=False, portType="In")
        self.currentRigGraph.getOrCreateArgument("rigScale", dataType="Float32", defaultValue=1.0, portType="In")
    ...
    def addAbstractBone(self, bone):
        self.__abstractBones.append(bone)
    ...
    def _preBuild(self, kSceneItem):
        ...
        self.__abstractBones = []
        self.__abstractConstraints = []
    ...
    def _postBuild(self, kSceneItem):
        ...
        """
        self.connectDccNodes()
        super(Builder, self)._postBuild(kSceneItem)
        # self.currentRigGraph.implodeNodesByGroup()
        # self.currentRigGraph.removeUnpluggedPort()
