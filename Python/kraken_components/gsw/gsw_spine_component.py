...
class GswSpineComponent(BaseExampleComponent):
    """Spine Component"""

    def __init__(self, name="spineBase", parent=None, *args, **kwargs):
        super(GswSpineComponent, self).__init__(name, parent, *args, **kwargs)
        ...
        self.orientationLockInputAttr = self.createInput('orientationLock', dataType='Float', value=1.0, parent=self.cmpInputAttrGrp).getTarget()

        # Declare Output Attrs


class GswSpineComponentGuide(GswSpineComponent):
    ...
    def __init__(self, name='spine', parent=None, *args, **kwargs):
        ...
        super(GswSpineComponentGuide, self).__init__(name, parent, *args, **kwargs)
        ...
        data = {
            'name': name,
            'location': 'M',
            'cogPosition': Vec3(0.0,  8.7, -0.41),
            'spine01Position': Vec3(0.0,  8.7, -0.41),
            'spine02Position': Vec3(0.0,  9.4, -0.30),
            'spine03Position': Vec3(0.0, 10.1, -0.20),
            'spine04Position': Vec3(0.0, 11.0, -0.22),
            'numDeformers': 6
        }
    ...
    def saveData(self):
        ...
        data = super(GswSpineComponentGuide, self).saveData()
    ...
    def loadData(self, data):
        ...
        super(GswSpineComponentGuide, self).loadData(data)
    ...
    def getRigBuildData(self):
        ...
        data = super(GswSpineComponentGuide, self).getRigBuildData()
    ...
    def getRigComponentClass(cls):
        ...
        return GswSpineComponentRig


class GswSpineComponentRig(GswSpineComponent):
    """Spine Component"""

    def __init__(self, name="spine", parent=None):

        Profiler.getInstance().push("Construct Spine Rig Component:" + name)
        super(GswSpineComponentRig, self).__init__(name, parent)

        # =========
        # Controls
        # =========
        # COG
        self.cogCtrlSpace = CtrlSpace('spine', parent=self.ctrlCmpGrp)

        # Pelvis
        self.pelvisCtrlSpace = CtrlSpace('pelvis', parent=self.cogCtrlSpace)
        self.pelvisCtrl = Control('pelvis', parent=self.pelvisCtrlSpace)
        self.pelvisCtrl.setCurveData(icon.compas)
        self.pelvisCtrl.alignOnYAxis(negative=True)
        self.pelvisCtrl.scalePoints(Vec3(1.25, 1.0, 1.0))
        self.pelvisCtrl.lockScale(True, True, True)

        # Spine01
        self.spine01CtrlSpace = CtrlSpace('spine01', parent=self.pelvisCtrl)
        ...
        self.spine02Ctrl = Control('spine02', parent=self.spine02CtrlSpace, shape="sphere")
        self.spine02Ctrl.scalePoints(Vec3(1.5, 1.5, 1.5))
        self.spine02Ctrl.lockScale(True, True, True)
        self.spine02Ctrl.setColor("blue")

        # Spine04
        self.spine04CtrlSpace = CtrlSpace('spine04', parent=self.cogCtrlSpace)
        self.spine04Ctrl = Control('spine04', parent=self.spine04CtrlSpace)
        self.spine04Ctrl.setCurveData(icon.compas)
        self.spine04Ctrl.scalePoints(Vec3(1.25, 1.0, 1.0))
        self.spine04Ctrl.lockScale(True, True, True)

        # Spine03
        self.spine03CtrlSpace = CtrlSpace('spine03', parent=self.spine04Ctrl)
        self.spine03Ctrl = Control('spine03', parent=self.spine03CtrlSpace, shape="sphere")
        self.spine03Ctrl.scalePoints(Vec3(1.5, 1.5, 1.5))
        ...
        # ==============
        # Constraint inputs
        self.spineSrtInputConstraint = PoseConstraint('_'.join([self.cogCtrlSpace.getName(), 'To', self.cogInputTgt.getName()]))
        self.spineSrtInputConstraint.addConstrainer(self.cogInputTgt)
        ...
        self.pelvisOutputConstraint.setMaintainOffset(True)
        ...
        # ===============
        # Add Spine Splice Op
        self.bezierSpineKLOp = KLOperator('spineKLOp', 'GswBezierSpineSolver', 'GSW_Kraken')
        ...
        self.bezierSpineKLOp.setInput('orientationLock', self.orientationLockInputAttr)
    ...
    def loadData(self, data=None):
        ...
        super(GswSpineComponentRig, self).loadData(data)
        ...
        self.pelvisCtrlSpace.xfo.tr = spine02Position
        self.pelvisCtrl.xfo.tr = spine02Position
        self.pelvisHeight = spine01Position.subtract(spine02Position)
        self.pelvisCtrl.translatePoints(self.pelvisHeight * Vec3(0, 1.5, 0))
        self.pelvisOutputTgt.xfo.tr = spine01Position
        ...
        self.orientationLockInputAttr.setValue(1.0)
...
ks.registerComponent(GswSpineComponentGuide)
ks.registerComponent(GswSpineComponentRig)
